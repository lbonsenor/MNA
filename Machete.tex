...
$$F(x) = \frac{a_0}{2} + \sum_{n=1}^{\infty} \left( a_n \cos(\omega_n x) + b_n \operatorname{sen}(\omega_n x) \right)$$
\begin{itemize}[leftmargin=1.2em]
\item $\omega_n = \dfrac{2\pi n}{L}$
\item $\dfrac{a_0}{2} = \dfrac{1}{L} \int_{L} F(x) \, dx$
\item $a_n = \dfrac{2}{L} \int_{L} F(x) \cos(\omega_n x) \, dx$
\item $b_n = \dfrac{2}{L} \int_{L} F(x) \operatorname{sen}(\omega_n x) \, dx$
\end{itemize}

\textbf{Serie Exponencial:}
$$F(x) = \sum_{n=-\infty}^{\infty} C_n e^{i \omega_n x}$$
\begin{itemize}[leftmargin=1.2em]
\item $C_n = \dfrac{a_n - i b_n}{2} = \dfrac{1}{L} \int_{L} F(x) e^{-i \omega_n x} \, dx$
\end{itemize}
...
\item $P_n = \sqrt{a_n^2 + b_n^2}$
\item $\tan(\varphi_n) = -\dfrac{b_n}{a_n}$
\item $a_n \cos\theta + b_n \operatorname{sen}\theta = P_n \cos(\theta + \varphi_n)$
\end{itemize}
\end{term}
...
\begin{itemize}[leftmargin=1.2em]
\item $F'(x) = \sum_{n=1}^{\infty} \left( \omega_n b_n \cos(\omega_n x) - \omega_n a_n \operatorname{sen}(\omega_n x) \right)$
\item $\int_{0}^{x} F(t) \, dt = \dfrac{a_0 x}{2} + \sum_{n=1}^{\infty} \left( -\dfrac{b_n}{\omega_n} \cos(\omega_n x) + \dfrac{a_n}{\omega_n} \operatorname{sen}(\omega_n x) \right)$
\end{itemize}
...
\begin{term}
    \section*{SISTEMA MATRICIAL IMPLICITO: $r=\frac{\Delta t}{\Delta x^2}\leq\frac{1}{2}$}
    $$\begin{pmatrix}
        1+2r & -r & 0 & 0 \\
        -r & 1+2r & -r & 0 \\
        0 & -r & 1+2r & -r \\
...
\begin{term}
    \section*{SISTEMA MATRICIAL EXPLICITO $r=\frac{\Delta t}{\Delta x^2}\leq\frac{1}{2}$}
...
\section*{DIFERENCIAS FINITAS IMPLÍCITAS (NEUMANN)}
\begin{multicols}{2}
Sea la ecuación: $\dfrac{\partial u}{\partial t} = \dfrac{\partial^2 u}{\partial x^2}$ con $u_x(0,t)=u_x(1,t)=0$.
\textbf{Inicializaciones y Grilla:}
\begin{itemize}[leftmargin=1.2em]
    \item Nodos internos $n = 4 \implies \Delta x = \dfrac{1 - 0}{4 + 1} = 0.2$.
\end{itemize}
\textbf{Esquema General para Nodos Internos:}
$$-r u_{i-1}^{j+1} + (1 + 2r)u_i^{j+1} - r u_{i+1}^{j+1} = u_i^j$$
\begin{itemize}[leftmargin=1.2em]
    \item En $x_0 = 0$: $\dfrac{u_1^{j+1} - u_{-1}^{j+1}}{2\Delta x} = 0 \implies u_{-1}^{j+1} = u_1^{j+1}$
    \item En $x_5 = 1$: $\dfrac{u_6^{j+1} - u_4^{j+1}}{2\Delta x} = 0 \implies u_6^{j+1} = u_4^{j+1}$
\end{itemize}
Sustituyendo en las ecuaciones de los extremos:
\begin{itemize}[leftmargin=1.2em]
    \item Fila $i=0$: $(1 + 2r)u_0^{j+1} - 2r u_1^{j+1} = u_0^j$
    \item Fila $i=5$: $-2r u_4^{j+1} + (1 + 2r)u_5^{j+1} = u_5^j$
\end{itemize}\textbf{Condición Inicial ($j = 0$):}
Evaluando $u(x,0) = x + 1$ en cada nodo $x_i$:$$u^0 = 
\begin{pmatrix} 
...
\end{multicols}
\begin{term}
\textbf{Resultado Final (Sistema Matricial $6 \times 6$):}
$$\begin{pmatrix}
1+2r & -2r & 0 & 0 & 0 & 0 \\
-r & 1+2r & -r & 0 & 0 & 0 \\
0 & -r & 1+2r & -r & 0 & 0 \\
...
\end{pmatrix}
\begin{pmatrix}
u_0^{j+1} \\ u_1^{j+1} \\ u_2^{j+1} \\ u_3^{j+1} \\ u_4^{j+1} \\ u_5^{j+1}
\end{pmatrix}\begin{pmatrix}u_0^j \\ u_1^j \\ u_2^j \\ u_3^j \\ u_4^j \\ u_5^j\end{pmatrix}$$\end{term}
